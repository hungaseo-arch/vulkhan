...
**언어.** 식별자와 주석은 인도네시아어입니다(`pelanggan`, `gudang`, `say()`).
문서(README, CHANGELOG 등)는 한국어입니다. 새 코드도 이 규칙을 따릅니다.

**UI 문자열.** 화면에 보이는 문자열은 전부 `t()`를 거칩니다. `useLang()`을 쓰는 컴포넌트라면
맨 윗줄에 `const { t } = useLang();`이 있어야 합니다.

- **사전 키가 곧 인도네시아어 원문**입니다. `t("Simpan")`처럼 쓰고, 한국어 값을
  [src/i18n.jsx](src/i18n.jsx)의 `KO`에 추가합니다. 번역을 빠뜨리면 인도네시아어가 그대로 나옵니다.
- 값이 들어가는 문장은 문자열을 쪼개지 말고 `{이름}` 자리표시자를 씁니다:
  `t("{n} hari", { n: c.termin })`. 한국어와 인도네시아어는 어순이 달라 조각을 이어 붙이면 깨집니다.
- 같은 인도네시아어가 뜻이 다르면 `"키|문맥"`으로 구분합니다. `|` 뒤는 화면에 절대 나오지 않습니다.
  현재 쓰는 것: `Masuk`(입고) / `Masuk|login`(로그인), `Keluar|mutasi`(출고) / `Keluar`(로그아웃),
  `Penawaran`(견적) / `Penawaran|dok`(견적서).
- `SO_LABEL`, `PO_LABEL`, `KATEGORI`, `ROLE_LABEL`, `TIPE_LABEL` 같은 상수는 인도네시아어 원문을
  담고, 쓰는 쪽에서 `t(SO_LABEL.kirim.id)`처럼 감쌉니다. 상수 자체를 번역하지 마세요.
- 엑셀 내보내기 머리글도 `t()`를 씁니다. 반면 **출력용 faktur·penawaran 서류 본문은 번역하지 않습니다** —
  인도네시아 거래처에 나가는 서류이므로 인도네시아어로 고정입니다. 상단 조작 버튼만 번역 대상입니다.
- 날짜 표기는 `tglPanjang(tgl, lang)` / `bulanLabel(ym, lang)`에 `lang`을 넘겨야 한국어 형식이 됩니다.
  서류 본문에서는 `lang`을 넘기지 않아 인도네시아어 형식으로 남습니다.

**로고.** `public/ascendo-symbol.png`(심볼), `public/ascendo-signature.png`(시그니처)는 아센도 공식
자산입니다. 헤더·로그인·파비콘에만 쓰고, 인쇄 서류 kop에는 넣지 않습니다. 그 서류의 발행처는
`PENERBIT`의 CV. Sinar Perkasa Ban / PT. Daimond Fajar Jaya이며, 아센도 로고를 넣으면 발행 주체가
잘못 표시됩니다.
...
- [ ] 엑셀 내보내기 파일명에 오늘 날짜
- [ ] 헤더 사용자 메뉴가 바깥 클릭·ESC로 닫히고, 비밀번호 변경·로그아웃이 동작
- [ ] `ID / KO` 전환이 모든 탭·모달·토스트에 반영되고, 새로고침 후에도 선택이 유지
- [ ] 한국어 상태에서 faktur·penawaran 출력물 본문이 인도네시아어 그대로인지
- [ ] 한국어 상태에서 날짜가 `2026년 9월 6일` 형식, 인도네시아어에서 `6 September 2026` 형식
